...
Playwright tests for the connection flow of the app in `template/`. They run
...
To run them by hand, scaffold an app, copy `auth-flow.spec.ts` and
`fake-indexer.ts` into its `e2e/` and `playwright.config.ts` over its own, then
`bun run e2e` there. Or run `bun run test:scaffold --keep` from the repo root.
