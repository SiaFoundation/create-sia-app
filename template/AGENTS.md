...
- **recovery** — The user generates or enters a phrase; `register(phrase)` calls `request.register()` and finishes with the `Sdk`. A registration error is shown with **Start over** only, because that request cannot register again. When approval lands, `request.reconnecting()` says whether this account has used the app before; if so, the screen asks for the existing phrase first, and `register(phrase)` checks it with `request.matchesExistingAppKey(phrase)`. A phrase that does not match stops at `newAccountPhrase`, because registering it would create a second, empty account, and the user confirms with **Create a new account** or goes back.
...
Installs with bun skip package versions published in the last three days (`bunfig.toml`), except the Sia SDK and its platform packages; npm has no equivalent setting. The dev server uses port 5173 and the preview server 4173, and each exits instead of moving to another port when that one is taken.
